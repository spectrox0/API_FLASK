...
confirmed_cases_data_url = base_url_api_covid + \
    'time_series_covid19_confirmed_global.csv'
death_cases_data_url = base_url_api_covid + \
    'time_series_covid19_deaths_global.csv'
recovery_cases_data_url = base_url_api_covid + \
...
def convert_dataframe(raw_data, list_countries=None, days=30):
    ...
    df = raw_data.groupby(
        'Country/Region').sum().drop(['Lat', 'Long'], axis=1).transpose()
